  ...
  describe '/schedules' do
    ...
    context '[POST] /schedules #schedules#create' do
      it 'returns a valid 201 with valid request' do
        ...
        post(
          ...
          params: {
            schedule: {
              ...
              frequency: bld_schedule.frequency,
              repeat_time: bld_schedule.repeat_time,
        ...
        expect(res_body['data']['frequency']).to eq(schedule.frequency)
        expect(res_body['data']['repeat_time']).to eq(schedule.repeat_time)
    ...
    context '[GET] /schedules #schedules#index' do
      it 'returns a valid 201 with valid request' do
        ...
        expect(res_body['data'][0]['frequency']).to eq(schedule.frequency)
        expect(res_body['data'][0]['repeat_time']).to eq(schedule.repeat_time)
    ...
    context '[PUT] /schedules/{schedule_id} #schedules#update' do
      it 'returns a valid 200 with valid request' do
        ...
        update_params = {
          frequency: 'yearly',
          repeat_time: bld_schedule.repeat_time + 1,
        ...
        expect(res_body['data']['id']).to eq(schedule.id)
        expect(res_body['data']['job_id']).to eq(schedule.job_id)
        expect(res_body['data']['frequency']).to eq(update_params[:frequency])
        expect(res_body['data']['repeat_time']).to eq(update_params[:repeat_time])
    ...
    context '[DELETE] /schedules/{schedule_id} #schedules#destroy' do
      it 'returns a valid 200 with valid request' do
        ...
        expect(res_body['data']['frequency']).to eq(crt_schedule.frequency)
        expect(res_body['data']['repeat_time']).to eq(crt_schedule.repeat_time)

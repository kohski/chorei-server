...
RSpec.describe Step, type: :model do
  ...
  context 'auto_numbering_blank_order method' do
    it 'works validly' do
      step = create(:step)
  ...
  context 'auto_renumbering_order_after_create method' do
    it 'works validly' do
      step_first
      step_second
      step_third
  ...
  # context 'auto_renumbering_order after update' do
  #   it 'works validly' do
  #     step_order_first
  #     step_order_second
  #     step_order_third
  ...
  #     # Although order of (third and second) is 1, second will be 2. because second.update is older then third.update
  #     step_order_third.update(order: 1)

  #     first = Step.find_by(memo: step_order_first.memo)
  #     second = Step.find_by(memo: step_order_second.memo)
  #     third = Step.find_by(memo: step_order_third.memo)
  #     fource = Step.find_by(memo: step_order_fource.memo)

  #     expect(first.order).to eq(Step.first.order)
  #     expect(second.order).to eq(Step.third.order)
  #     expect(third.order).to eq(Step.second.order)
  #     expect(fource.order).to eq(Step.fource.order)
  #   end
  # end
end
